// /assets/js/pengeluaran-tetap.js
...
function loadDataRealtime() {
    // Gunakan koleksi "pengeluaran_tetap"
    const q = collection(db, "pengeluaran_tetap");
    ...
    onSnapshot(q, (snapshot) => {
      ...
      `;
        ...
        console.log("DataTables re-initialized for Pengeluaran Tetap"); // DEBUG
    });
}
...
document.getElementById("selectAll").addEventListener("change", (e) => {
    $('#tablePengeluaran').find('tbody .rowCheckbox').prop('checked', e.target.checked);
});
...
document.getElementById("deleteSelectedBtn").addEventListener("click", async () => {
    const selectedIds = $('#tablePengeluaran').find('tbody .rowCheckbox:checked').map(function () {
        return this.value;
    }).get();
    ...
    if (res.isConfirmed) {
        for (const id of selectedIds) {
            // Hapus dari koleksi "pengeluaran_tetap"
            await deleteDoc(doc(db, "pengeluaran_tetap", id));
        }
...
document.getElementById("exportExcelBtn").addEventListener("click", async () => {
    ...
    // Ambil dari koleksi "pengeluaran_tetap"
    const promises = selectedIds.map(id => getDoc(doc(db, "pengeluaran_tetap", id)));
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Pengeluaran Tetap");

    XLSX.writeFile(wb, `Pengeluaran_Tetap_Selected.xlsx`);
...
document.getElementById("pengeluaranForm").addEventListener("submit", async (e) => {
    ...
    try {
        // Simpan ke koleksi "pengeluaran_tetap"
        await addDoc(collection(db, "pengeluaran_tetap"), { keterangan, kuantitas, jumlah, tanggal, uploader });
        e.target.reset();
